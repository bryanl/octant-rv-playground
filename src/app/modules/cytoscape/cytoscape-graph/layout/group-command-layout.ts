...
class SyntheticEdgeGenerator {
  ...
  private static normalizeToParent(element: any): any {
    return element.isChild() ? element.parent() : element;
  }

  public getEdge(source: Cy.NodeSingular, target: Cy.NodeSingular): Cy.EdgeDefinition | null {
    const sourceId = SyntheticEdgeGenerator.normalizeToParent(source).id();
    const targetId = SyntheticEdgeGenerator.normalizeToParent(target).id();
...
function Layout(options: any): void {
...
Layout.prototype.run = function run(): void {
  ...
  parents.each(parent => {
    ...
    compoundLayout.on('layoutstart layoutready layoutstop', _ => {
  ...
  children.each(child => {
    child.connectedEdges().each(edge => {
      ...
      const syntheticEdge = this.gen.getEdge(edge.source(), edge.target());
      if (syntheticEdge) {
...
Layout.prototype.parents = function parents(): Cy.NodeCollection {
  return this.elements.nodes('$node > node');
};

export default function register(cytoscape): void {
  cytoscape('layout', 'group-compound-layout', Layout);
}
